...
#         'BRL':13,}
    ...
    if request.method == "GET":
        ...
        return render(request, 'core/main_page.html', {'form':form,'csvreader':get_currencies()})
    if request.method == 'POST':
        form = QuantyToConvert(request.POST)
        if form.is_valid():
            ...
            converted_sum = round((get_currencies()[f'USD{curr_to}']/(get_currencies()[f'USD{curr_from}']))*float(amount_of_money),2)
